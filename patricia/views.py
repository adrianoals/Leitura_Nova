...
def patricia(request):
    if request.method == 'POST':
        ...
        valor_leitura_cozinha = request.POST.get('valor_leitura_cozinha')
        foto_relogio_cozinha = request.FILES.get('foto_relogio_cozinha')
        valor_leitura_banheiro = request.POST.get('valor_leitura_banheiro')
        foto_relogio_banheiro = request.FILES.get('foto_relogio_banheiro')

        try:
            if not all([apartamento_id, valor_leitura_cozinha, valor_leitura_banheiro ,foto_relogio_cozinha, foto_relogio_banheiro]):
                raise ValueError('Todos os campos são obrigatórios.')

            apartamento = Apartamento.objects.get(id=apartamento_id)
            Leitura.objects.create(
                apartamento=apartamento,
                valor_leitura_cozinha=valor_leitura_cozinha,
                valor_leitura_banheiro=valor_leitura_banheiro,
                data_leitura=datetime.date.today(),  # Supondo que a data da leitura é sempre o dia atual
                foto_relogio_cozinha=foto_relogio_cozinha, 
                foto_relogio_banheiro=foto_relogio_banheiro
            )
    ...
    return render(request, 'patricia/patricia.html', {'apartamentos': apartamentos})


def download_photos(request):
    # Diretório onde as fotos estão armazenadas
    photos_directory = os.path.join(settings.MEDIA_ROOT, 'leituras/patricia/03')
