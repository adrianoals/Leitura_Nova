...
def rename_file_cozinha(instance, filename):
    # Obter o mês atual
    month = now().strftime('%m')
    # Construir o novo nome do arquivo. Você pode ajustar o formato conforme necessário
    new_filename = f"{instance.apartamento.apartamento}_mês_{month}_cozinha.jpg"
    # Retornar o caminho completo do novo arquivo
    return f"leituras/patricia/{month}/{new_filename}"

def rename_file_banheiro(instance, filename):
    # Obter o mês atual
    month = now().strftime('%m')
    # Construir o novo nome do arquivo. Você pode ajustar o formato conforme necessário
    new_filename = f"{instance.apartamento.apartamento}_mês_{month}_banheiro.jpg"
    # Retornar o caminho completo do novo arquivo
    return f"leituras/patricia/{month}/{new_filename}"
...
class Leitura(models.Model):
    ...
    foto_relogio_cozinha = models.ImageField(upload_to=rename_file_cozinha, blank=True, null=True)
    foto_relogio_banheiro = models.ImageField(upload_to=rename_file_banheiro, blank=True, null=True)
